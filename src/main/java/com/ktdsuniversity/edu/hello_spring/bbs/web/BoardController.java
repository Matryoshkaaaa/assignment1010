...
public class BoardController {
	...
	public String doCreateNewBoard(@Valid WriteBoardVO writeBoardVO // @Valid WriteBoardVO의 Validation
			, BindingResult bindingResult // @Valid의 실패 결과만 할당 받는다.
			, Model model, @SessionAttribute(value = "_LOGIN_USER_", required = false) MemberVO loginMemberVO) { // LOGIN // USER의 // 회원정보를 // 가져와라
		...
		/*
		 * session에서 가져온 MemberVO 인스턴스는 로그인 여부에 따라 NULL 혹은 인스턴스가 할당되어 있다. memberVO가
		 * NULL이라면, 로그인이 안되어있는 것으로 로그인을 유도시켜야 한다.
		 */
